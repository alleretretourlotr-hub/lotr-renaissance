...
/** GENERE PAR L'USINE v2 - recruteur, port de LOTREntityAngmarHillmanChieftain (table ANGMAR_HILLMAN_CHIEFTAIN) */
public class LOTREntityAngmarHillmanChieftain extends LOTREntityNPC {

    public LOTREntityAngmarHillmanChieftain(EntityType<? extends LOTREntityAngmarHillmanChieftain> type, World world) {
        super(type, world);
    }
    ...
    protected net.minecraft.util.ActionResultType mobInteract(
        ...
        if (hand == net.minecraft.util.Hand.MAIN_HAND && !player.isShiftKeyDown()) {
            ...
            if (alignment < fr.alleretretour.lotr.hire.LOTRHireRosters.ANGMARHILLMANCHIEFTAIN_BASE) {
                return super.mobInteract(player, hand);
            }
            if (!level.isClientSide
                    && player instanceof net.minecraft.entity.player.ServerPlayerEntity) {
                final LOTREntityAngmarHillmanChieftain self = this;
                player.openMenu(new net.minecraft.inventory.container.INamedContainerProvider() {
                    ...
                    public net.minecraft.inventory.container.Container createMenu(
                        ...
                        return new fr.alleretretour.lotr.inventory.LOTRContainerHire(id, inv,
                                fr.alleretretour.lotr.hire.LOTRHireRosters.angmarHillmanChieftain(), self);
                    }
                });
            }
