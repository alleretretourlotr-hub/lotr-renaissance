...
/** GENERE (mega-lot Phase 4) - port de lotr.common.entity.npc.LOTREntityNearHaradrimWarrior */
public class LOTREntityNearHaradrimWarrior extends LOTREntityNPC {

    public LOTREntityNearHaradrimWarrior(EntityType<? extends LOTREntityNearHaradrimWarrior> type, World world) {
        super(type, world);
    }
    ...
    /** PORT de onSpawnWithEgg (Legacy LOTREntityNearHaradrimWarrior) : pool d'armes + armure exacts. */
    ...
    public net.minecraft.entity.ILivingEntityData finalizeSpawn(
        ...
        String[] pool = {"lotr:harad_spear", "lotr:near_harad_spear"};
        setItemSlot(net.minecraft.inventory.EquipmentSlotType.MAINHAND,
                stackOf(pool[getRandom().nextInt(pool.length)]));
        setItemSlot(net.minecraft.inventory.EquipmentSlotType.HEAD, stackOf("lotr:near_harad_helmet"));
