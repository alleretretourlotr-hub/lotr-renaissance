...
/** GENERE PAR L'USINE v2 - recruteur, port de LOTREntityMoredainChieftain (table MOREDAIN_CHIEFTAIN) */
public class LOTREntityMoredainChieftain extends LOTREntityNPC {

    public LOTREntityMoredainChieftain(EntityType<? extends LOTREntityMoredainChieftain> type, World world) {
        super(type, world);
    }
    ...
    protected net.minecraft.util.ActionResultType mobInteract(
        ...
        if (hand == net.minecraft.util.Hand.MAIN_HAND && !player.isShiftKeyDown()) {
            ...
            if (alignment < fr.alleretretour.lotr.hire.LOTRHireRosters.MOREDAINCHIEFTAIN_BASE) {
                return super.mobInteract(player, hand);
            }
            if (!level.isClientSide
                    && player instanceof net.minecraft.entity.player.ServerPlayerEntity) {
                final LOTREntityMoredainChieftain self = this;
                player.openMenu(new net.minecraft.inventory.container.INamedContainerProvider() {
                    ...
                    public net.minecraft.inventory.container.Container createMenu(
                        ...
                        return new fr.alleretretour.lotr.inventory.LOTRContainerHire(id, inv,
                                fr.alleretretour.lotr.hire.LOTRHireRosters.moredainChieftain(), self);
                    }
                });
            }
    ...
    public net.minecraft.entity.ILivingEntityData finalizeSpawn(
        ...
        String[] pool = {"lotr:moredain_battleaxe"};
        setItemSlot(net.minecraft.inventory.EquipmentSlotType.MAINHAND,
                stackOf(pool[getRandom().nextInt(pool.length)]));
        setItemSlot(net.minecraft.inventory.EquipmentSlotType.HEAD, stackOf("lotr:moredain_lion_helmet"));
        setItemSlot(net.minecraft.inventory.EquipmentSlotType.CHEST, stackOf("lotr:moredain_lion_chestplate"));
        setItemSlot(net.minecraft.inventory.EquipmentSlotType.LEGS, stackOf("lotr:moredain_lion_leggings"));
        setItemSlot(net.minecraft.inventory.EquipmentSlotType.FEET, stackOf("lotr:moredain_lion_boots"));
