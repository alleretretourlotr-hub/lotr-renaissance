...
/** GENERE PAR L'USINE v2 - recruteur, port de LOTREntityNearHaradrimWarlord (table NEAR_HARADRIM_WARLORD) */
public class LOTREntityNearHaradrimWarlord extends LOTREntityNPC {

    public LOTREntityNearHaradrimWarlord(EntityType<? extends LOTREntityNearHaradrimWarlord> type, World world) {
        super(type, world);
    }
    ...
    protected net.minecraft.util.ActionResultType mobInteract(
        ...
        if (hand == net.minecraft.util.Hand.MAIN_HAND && !player.isShiftKeyDown()) {
            ...
            if (alignment < fr.alleretretour.lotr.hire.LOTRHireRosters.NEARHARADRIMWARLORD_BASE) {
                return super.mobInteract(player, hand);
            }
            if (!level.isClientSide
                    && player instanceof net.minecraft.entity.player.ServerPlayerEntity) {
                final LOTREntityNearHaradrimWarlord self = this;
                player.openMenu(new net.minecraft.inventory.container.INamedContainerProvider() {
                    ...
                    public net.minecraft.inventory.container.Container createMenu(
                        ...
                        return new fr.alleretretour.lotr.inventory.LOTRContainerHire(id, inv,
                                fr.alleretretour.lotr.hire.LOTRHireRosters.nearHaradrimWarlord(), self);
                    }
                });
            }
    ...
    public net.minecraft.entity.ILivingEntityData finalizeSpawn(
        ...
        String[] pool = {"lotr:near_harad_poleaxe"};
        setItemSlot(net.minecraft.inventory.EquipmentSlotType.MAINHAND,
                stackOf(pool[getRandom().nextInt(pool.length)]));
        setItemSlot(net.minecraft.inventory.EquipmentSlotType.HEAD, stackOf("lotr:near_harad_warlord_helmet"));
        setItemSlot(net.minecraft.inventory.EquipmentSlotType.CHEST, stackOf("lotr:near_harad_chestplate"));
        setItemSlot(net.minecraft.inventory.EquipmentSlotType.LEGS, stackOf("lotr:near_harad_leggings"));
        setItemSlot(net.minecraft.inventory.EquipmentSlotType.FEET, stackOf("lotr:near_harad_boots"));
