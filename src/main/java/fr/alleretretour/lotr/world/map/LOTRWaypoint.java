...
/**
 * PORT de lotr.common.world.map.LOTRWaypoint : les 274 points de passage
 * de la Terre du Milieu (region, faction, coordonnees carte, masque ou non).
 * Les coordonnees x / z sont celles de la carte (pixels de map.png) ; la
 * conversion vers le monde reprend l'origine et l'echelle de la generation.
 */
public enum LOTRWaypoint {
    ...
    /** coordonnees SUR LA CARTE (pixels de map.png), valeurs du Legacy */
    public final double mapX;
    public final double mapZ;
    /** coordonnees dans le MONDE (blocs), converties a la construction */
    public final int x;
    public final int z;
    /** point non liste (PORT du 5e parametre du Legacy) */
    public final boolean hidden;

    LOTRWaypoint(Region region, LOTRFaction faction, double x, double z) {
        this(region, faction, x, z, false);
    }

    LOTRWaypoint(Region region, LOTRFaction faction, double mapX, double mapZ, boolean hidden) {
        this.region = region;
        this.faction = faction;
        this.mapX = mapX;
        this.mapZ = mapZ;
        // PORT du calcul de LOTRGenLayerWorld : pixel de carte -> bloc
        this.x = (int) ((mapX - LOTRBiomeSource.ORIGIN_X) * LOTRBiomeSource.SCALE);
        this.z = (int) ((mapZ - LOTRBiomeSource.ORIGIN_Z) * LOTRBiomeSource.SCALE);
        this.hidden = hidden;
    }
